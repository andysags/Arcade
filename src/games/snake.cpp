...
std::vector<std::string> Snake::loadMap(const std::string &filename)
{
...
void Snake::init()
{
    this->map = this->loadMap("resources/snake.txt");
    this->reset();
...
const char *Snake::getGameName() const
{
    return "Snake";
}
...
void Snake::setGameOver(bool gameOver)
{
...
void Snake::setSnakeSize(int snakeSize)
{
...
Snake::pos Snake::getFood() const
{
    return this->food;
}

int Snake::game(IGraphic *module, int *bestscore)
{
    (void)bestscore;
    module->drawSprite(0, 0, "resources/assets/background.png");
    for (int i = 0; i < this->getSnakeSize(); i++) {
        module->drawSprite(800 + this->getSnake()[i].x * 20, 150 + this->getSnake()[i].y * 20, "resources/assets/player.png");
    }
    std::vector<std::string> map = this->getMap();
    for (size_t y = 0; y < map.size(); y++) {
        for (size_t x = 0; x < map[y].size(); x++) {
            if (map[y][x] == '#') {
                module->drawSprite(800 + x * 20, 150 + y * 20, "resources/assets/wall.png");
            } else if (map[y][x] == 'o') {
                module->drawSprite(800 + x * 20, 150 + y * 20, "resources/assets/apple.png");
            }
        }
    }
    return 1;
